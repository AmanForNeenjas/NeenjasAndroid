 ...
 *    desc   : 预充电量值设置为xy
 */
public final class DevSetDisChargingActivity extends AppActivity {
    ...
    public static void start(Context context, DeviceBind model, String minDischarging) {
        Intent intent = new Intent(context, DevSetDisChargingActivity.class);
    ...
    protected int getLayoutId() {
        return R.layout.dev_setdischarging_activity;
    }
    ...
    protected void initData() {
        ...
        try{
            setnum = Integer.parseInt(minDischarging);
        }catch (Exception e){
            setnum = 0;
        }
        if(setnum<=0 || setnum>100){
            mimplusView.setProgress(0);
            msetValView.setText( "0/100");
        }else{
    ...
    private void sendStatusAll() {
        ...
        step = 128;
        sendDevicebyAddrHexStr(RecodeUtil.getRegisterAddrSetMinDisCharging(RecodeUtil.int2HexString( setnum)), "V125");
    }
    /**
     * 通过 HexStr 向设备发送请求
     */
