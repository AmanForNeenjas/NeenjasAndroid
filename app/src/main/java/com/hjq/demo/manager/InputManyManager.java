 ...
 *    time   : 2018/10/18
 *    desc   : 文本输入管理类，通过管理多个 EditText CheckBox输入是否为空来启用或者禁用按钮的点击事件
 *    blog   : https://www.jianshu.com/p/fd3795e8a6b3
 */
public final class InputManyManager implements TextWatcher, CompoundButton.OnCheckedChangeListener {
     ...
     */
    private InputManyManager(View view, boolean alpha) {
        if (view == null) {
    ...
    public void addRadio(CheckBox views){
        if (views == null) {
            return;
        }
        checkBox = views;
    ...
    public static final class Builder {
        ...
        public InputManyManager build() {
            InputManyManager helper = new InputManyManager(mView, isAlpha);
            helper.addViews(mViewSet);
            helper.addRadio(mCheckBox);
            helper.setListener(mListener);
            TextInputLifecycle.register(mActivity, helper);
            return helper;
        }

        public Builder addRadio(CheckBox checkBox) {
    ...
    private static class TextInputLifecycle implements Application.ActivityLifecycleCallbacks {
        ...
        private InputManyManager mTextHelper;

        private TextInputLifecycle(Activity activity, InputManyManager helper) {
            mActivity = activity;
            mTextHelper = helper;
        }

        private static void register(Activity activity, InputManyManager helper) {
    ...
    public interface OnInputTextListener {
         ...
         */
        boolean onInputChange(InputManyManager manager);
    }
}