...
// private RectF rect;//绘制范围
    ...
    public GoodProgressView(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.GoodProgressView_startColor) {// 渐变色之起始颜色，默认设置为红色
                mColors[0] = a.getColor(attr, Color.RED);
            } else if (attr == R.styleable.GoodProgressView_endColor) {// 渐变色之结束颜色，默认设置为品红
                mColors[1] = a.getColor(attr, Color.MAGENTA);
            } else if (attr == R.styleable.GoodProgressView_backgroundColor) {// 进度条默认颜色，默认设置为灰色
                backgroundColor = a.getColor(attr, Color.GRAY);
            } else if (attr == R.styleable.GoodProgressView_textColor) {// 文字颜色，默认设置为灰色
        ...
        if(progressValue<=20){
            mColors[0] = Color.RED;
            mColors[1] = Color.MAGENTA;
        }
        else{
            mColors[0] = Color.parseColor("#0066FF");
            mColors[1] = Color.parseColor("#96F9FF");
        }
        mPaint = new Paint();
        progressValue = 0;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(progressValue<=20){
            ...
            mColors[0] = Color.parseColor("#E6FF5B");
            mColors[1] = Color.parseColor("#02D67F");
        }
        //按比例计算进度条各部分的值
...
// float width = 300*unit;//绘画区域的长度
        ...
        float progressWidth = 250 * unit;//绘画区域的长度258
        ...
        float radius = offsetWidth * 2 + progressWidth;
        if(radius <=0){
            radius =0.1f;
        }
        ...
        //设置渐变色区域,线性渐变
        LinearGradient shader = new LinearGradient(0, 0, offsetWidth * 2 + progressWidth, 0, colors, new float[]{0, 1f}, Shader.TileMode.CLAMP);
        //放射渐变
        RadialGradient shader1 = new RadialGradient(0, 0, radius, colors, new float[]{0,  1f}, Shader.TileMode.CLAMP);
        //扫描渐变
        SweepGradient shader2 = new SweepGradient(0, 0, colors, new float[]{0,1f});
//        BitmapShader shader3 = new BitmapShader(BitmapFactory.decodeFile(""), Shader.TileMode.CLAMP, Shader.TileMode.CLAMP);
        //组合渐变
        ComposeShader shader4 = new ComposeShader(shader1, shader2, PorterDuff.Mode.DST_IN);
        ...
        mPaint.setTextSize(wordHeight*1.3f);
        //计算坐标使文字居中
